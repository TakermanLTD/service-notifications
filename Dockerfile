...
ARG NUGET_PASSWORD
ARG SLACK_WEB_HOOK
ENV SLACK_EXCEPTIONS=SLACK_WEB_HOOK

COPY . .

COPY ["takerman.notifications.client/nuget.config", "./"]
COPY ["takerman.notifications.client/package.json", "package.json"]
...
WORKDIR "/src/Takerman.Notifications.Server"
...
ENTRYPOINT ["dotnet", "Takerman.Notifications.Server.dll"]